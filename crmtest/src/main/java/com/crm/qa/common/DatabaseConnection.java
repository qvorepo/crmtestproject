...
public class DatabaseConnection
{
	...
	private String url = "jdbc:sqlserver://tst-cobornssql.ccofarlz80tm.us-east-1.rds.amazonaws.com:1433;DatabaseName=web_db;sendStringParametersAsUnicode=false"; 
    ...
    private Properties getPropValue() throws IOException {
    	props=new Properties();
     	try {
     		props.load(new FileInputStream("./Properties/CommonProps.properties"));
	...
	public String getNextUserName() {
		...
		query ="SELECT username FROM customer WHERE username like 'STTEST%' order by customer.username desc";
		...
		try
		{
			...
			while(rs.next())
			{
				...
				for(int i=1; i <=columnCount; i++)
				{
					...
					nextUserName=nextUserName.substring(0,6) + getNextUserNameSuffix(nextUserName) ;
	...
	public void deletePayment(String Username) {
		query ="DELETE FROM customer_payment_option WHERE username='"  + Username + "'";
